...
static int calculate_path(Graph* g,
                          int src,
                          int dst,
                          int* path,
                          int* path_len,
...
static void child_process_run(int traveler_index,
                              Graph* g,
                              int src,
                              int dst,
                              int write_fd) {
    ...
    for (int i = 0; i < path_len; i++) {
        ...
        int next_weight = next >= 0 ? edge_weight(g, current, next) : 0;

        send_pipe_message(write_fd, traveler_index, current, next, 0, WAITING);

        lock_node(current, traveler_index, next_weight);

        send_pipe_message(write_fd, traveler_index, current, next, 0, INSIDE_NODE);

        sleep(1);

        unlock_node(current, traveler_index);
...
static void init_traveler(M7Traveler* t, int src, int dst) {
    memset(t, 0, sizeof(*t));
...
static void spawn_ipc_child(Graph* g, M7Traveler* t, int index) {
    if (pipe(t->pipe_fd) < 0) {
        perror("pipe");
        exit(1);
    }
...
static void receive_pipe_updates(M7Traveler* travelers, int traveler_count) {
    for (int i = 0; i < traveler_count; i++) {
        ...
        while (1) {
            ...
            if (n == sizeof(msg)) {
                M7Traveler* t = &travelers[msg.traveler_index];
...
static void update_animation(Graph* g,
                             M7Traveler* travelers,
                             int traveler_count,
                             float dt) {
    for (int i = 0; i < traveler_count; i++) {
        M7Traveler* t = &travelers[i];
...
static void draw_all_travelers(Graph* g,
                               M7Traveler* travelers,
                               int count,
                               float* x,
                               float* y) {
...
static void cleanup_children(M7Traveler* travelers, int traveler_count) {
    for (int i = 0; i < traveler_count; i++) {
...
static int parse_schedule_args(int argc, char* argv[], ScheduleType* sched, const char** file_name) {
    if (argc != 4) {
        return 0;
    }
...
static const char* schedule_name(ScheduleType sched) {
    if (sched == SCHED_SJF) {
        return "SJF";
    }
...
int main(int argc, char* argv[]) {
    ScheduleType selected_schedule;
    const char* file_name = NULL;

    if (!parse_schedule_args(argc, argv, &selected_schedule, &file_name)) {
        fprintf(stderr, "Usage:\n");
        fprintf(stderr, "  %s -schd fcfs <file_name>\n", argv[0]);
        fprintf(stderr, "  %s -schd sjf <file_name>\n", argv[0]);
        return 1;
    }

    g_schedule_type = selected_schedule;

    GraphData* data = read_file(file_name);
    if (!data) return 1;
    ...
    M7Traveler travelers[MAX_TRAVELERS_LOCAL];
    ...
    while (!WindowShouldClose()) {
        ...
        DrawText("Milestone 7 - Scheduling", 20, 20, 24, BLACK);
        ...
        DrawText(sched_text, 20, 55, 20, BLUE);

        DrawText("FCFS chooses by arrival order. SJF chooses by shortest next edge.",
                 20, 82, 18, DARKGRAY);

        if (!started) {
            DrawText("Press SPACE to start", 20, 112, 20, DARKGREEN);
        } else {
