...
#define WAITING_COLOR   (Color){255, 230, 0,   255}
#define WAITING_BORDER  (Color){180, 130, 0,   255}
#define INSIDE_FILL     WHITE
#define INSIDE_GLOW     (Color){255, 255, 180, 120}
...
void drawArrivalMessage(int travelerIndex,
                        int dst,
                        int totalWeight,
...
void drawTraveler(int travelerIndex,
                  float px,
                  float py,
                  int dst,
                  int arrived,
    ...
    /*
     * Waiting travelers are drawn outside the node.
     * This makes multiple waiting travelers visible.
     */
    if (state == WAITING) {
        float spread = 30.0f;
        float sign = (slotIndex % 2 == 0) ? -1.0f : 1.0f;
        float mag = (float)(slotIndex / 2 + 1);
    ...
    if (arrived) {
        drawArrivalMessage(travelerIndex,
                           dst,
                           totalWeight,
                           stationNames);
    }
