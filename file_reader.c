...
GraphData* read_file(const char* filename) {
    ...
    /* Validation requirement: Max 15 nodes for GUI clarity */
    ...
    /* 2. Read all M edges (src, dst, weight) */
    for (int i = 0; i < data->m; i++) {
        if (fscanf(f, "%d %d %d", &data->edges[i].src, &data->edges[i].dst, &data->edges[i].weight) != 3) {
            printf("Error: Invalid edge format at line %d\n", i + 2);
            fclose(f); free(data->edges); free(data); return NULL;
        }
    ...
    /* 3. Read the total number of travelers (Crucial for Part 5 Pipe instantiation) */
    ...
    /* 4. Loop to read src and dst for all travelers */
    for (int i = 0; i < data->traveler_count; i++) {
        if (fscanf(f, "%d %d", &data->travelers[i].src, &data->travelers[i].dst) != 2) {
            ...
            fclose(f); free(data->travelers); free(data->edges); free(data); return NULL;
        }
