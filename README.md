...
### Milestone 7
- Added scheduling algorithms for travelers waiting to enter the same node
- Supported two scheduling policies: FCFS and SJF
- The scheduling policy is selected at runtime using the command line:
  - `./sim -schd fcfs graph.txt`
  - `./sim -schd sjf graph.txt`
...
In SJF, the traveler with the shortest next edge weight is selected first.
This can reduce the average waiting time, but travelers with longer next edges may wait more.
...
#### Ghada Bader – Synchronization and Node Locks
...
#### Miral Agha – Scheduling Algorithms
**Files:** `scheduler.c`, `scheduler.h`

Implemented the scheduling module for Milestone 7.
Added support for two scheduling algorithms: FCFS and SJF.
Created a scheduler type enum and a function that receives a waiting queue and returns the next traveler according to the selected scheduling policy.

#### Ghada Bader – Waiting Queues per Node
**Files:** `queue.c`, `queue.h`

Implemented a waiting queue for each graph node.
Added traveler information to the queue, including traveler id, arrival order, next edge weight, and waiting state.
Provided queue functions such as adding a traveler, removing a traveler, and accessing the waiting queue of a specific node.

#### Shahd Julani – Synchronization Between Processes
**Files:** `sync.c`, `sync.h`

Managed the synchronization mechanism between traveler processes.
Ensured that only one traveler can enter a node at a time.
Connected the synchronization logic with the selected scheduler so the parent process can choose which waiting traveler should enter next.
Updated traveler states to `WAITING` when a traveler is waiting to enter a node and helped prevent synchronization conflicts.

#### Shahid Hassan Haj – Main, GUI, Build, and Documentation
**Files:** `Main.c`, `path.c`, `drawing.c`, `drawing.h`, `README.md`, `Makefile`, `graph.txt`

Updated the main program to support scheduler selection from the command line:
...
Added command-line validation and passed the selected scheduling algorithm to the simulation.
Updated the GUI to clearly display the active scheduler, such as `Scheduler: FCFS` or `Scheduler: SJF`.
Updated the Makefile with the `milestone7` target, prepared a test input file, updated the README, tested the final integration, and created the final Git tag.
...
- Milestone 7 supports FCFS and SJF scheduling from the command line